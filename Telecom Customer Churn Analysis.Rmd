...
ggcorrplot(corr = Corr, method = "square", type = "lower")
#
library("PerformanceAnalytics")
chart.Correlation(r_mice, histogram=TRUE, pch=19)
...
fit2 = glm(churn ~ totmrc_Mean + vceovr_Mean + change_mou + 
    change_rev + drop_vce_Mean + drop_dat_Mean + blck_vce_Mean + 
    blck_dat_Mean + threeway_Mean + mou_cvce_Mean + mou_rvce_Mean + 
    owylis_vce_Mean + mou_opkv_Mean + drop_blk_Mean + months + 
    uniqsubs + actvsubs + avgrev + hnd_price + phones + eqpdays + 
...
sort(cooks.distance(fit2), decreasing = TRUE)[1:5]
```
...
```{r en}
# Elastic Net
cvfit = cv.glmnet(data.matrix(X_train), y_train, family = "binomial", alpha=0.5, type.measure = "class")
plot(cvfit)
cvfit$index
coef(cvfit, s=cvfit$lambda.min)
cmat = confusion.glmnet(cvfit, newx = data.matrix(X_test), newy = y_test)
cmat

bestbeta = coef(cvfit, s = "lambda.min")
...
barplot(coef$beta, names = coef$gene.names, las=2)
```
...
par(pty="s", mar=c(4,4,0.1,0.1)); plot(rocs[[best]], type = "l", main = "ROC")
...
```{r}
  library(ROCR)
pred1 = predict(cvfit, newx= data.matrix(X_test),
                   s="lambda.min", type="response")
pred2 = predict(cvfit1, newx= data.matrix(X_test),
                   s="lambda.min", type="response")
...
roc2 <- prediction(pred2, df.test$churn)
  ...
  perf2 <- performance(roc2,"tpr","fpr")
    ...
    legend("bottomright",legend=c("Lasso", "Ridge"), col=c("red", "blue"),lty = 1)
