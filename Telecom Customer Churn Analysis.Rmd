...
```{r}
tele_num_scale_imp$Customer_ID = rownames(tele_num_scale_imp) 
df = na.omit(merge(tele_num_scale_imp, raw_data, by = "Customer_ID"))
df1 = select(df, -c("Customer_ID"))
df1[grep("kid", names(df))] = ifelse(df1[grep("kid", names(df))] == "Y", 1, 0)
df1$kid0_2 = ifelse(df1$kid0_2 =="Y", 1, 0)
df1$infobase = ifelse(df1$infobase =="M", 1,0)
df1$asl_flag = ifelse(df1$asl_flag == "Y", 1, 0)
df1$refurb_new = ifelse(df1$refurb_new == "R", 1, 0)
df1$creditcd = ifelse(df1$creditcd =="Y", 1, 0)
df1$hnd_webcap = ifelse(df1$hnd_webcap == "WCMB", 1, 0)
df1$ownrent = ifelse(df1$ownrent == "R", 1, 0)
df1$dwlltype = ifelse(df1$dwlltype == "M", 1,0)
categorical = colnames(df1[, sapply(df1, class) == "character"])
df1[, categorical] = lapply(df1[, categorical], factor)
```

```{r}
X = data.matrix(df1[, -df1$churn])
Y = df1[,df1$churn]
set.seed(1)
s = sample(nrow(df1), nrow(df1)*0.6)
df0 = df1[s,]
xtrain = data.matrix(df0[s,-df0$churn])
ytrain = df0[s,df0$churn]
xtest = data.matrix(df0[-s,-df0$churn])
ytest = df0[-s,df0$churn]
```
...
cvfit = cv.glmnet(X, Y, family = "binomial", alpha=0.4, type.measure = "class")
...
```{r}
cvfit1 = cv.glmnet(xtrain, ytrain, family = "binomial", alpha=0.2,
                    nlambda = 30, type.measure="auc", keep=TRUE)
...
rf.fit = randomForest(X, as.factor(Y), ntree = 1000, 
                        mtry = 2, nodesize = 10, sampsize = 300,
                        importance=TRUE)
```
