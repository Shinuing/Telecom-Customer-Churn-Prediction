...
#assume 10-fold cv
loglik.fit <- cv.glmnet(x = as.matrix(X_train), y = y_train, alpha = 0.5, nfold = 5, family = binomial)
plot(loglik.fit)
```
...
library(glmnet)
# Elastic Net
cvfit = cv.glmnet(data.matrix(X_train), y_train, family = "binomial", alpha=0.5, type.measure = "class",nfold = 10)
plot(cvfit)
cvfit$index
...
#ROC
library(glmnet)
cvfit1 = cv.glmnet(data.matrix(X_train), y_train, family = "binomial", nlambda = 28, type.measure="auc", keep=TRUE, nfold = 5)
cvfit1$index
...
# set alpha's domain 
alpha0 = seq(0, 1, 0.2)
# create a empty list to store cross-validation errors
cv_error = NULL
# a loop to find the optimal alpha
for (i in 1:length(alpha0)){
  lasso0 = cv.glmnet(data.matrix(X_train), y_train,
                     nfolds = 10, 
                     alpha = alpha0[i], 
                     family = "binomial"
                     ,type.measure = "auc")
  cv_error[i]=min(lasso0$cvm)
}
alpha0[which.max(cv_error)]
```


```{r}
cvfit0= cv.glmnet(data.matrix(X_train), y_train, family = "binomial", alpha = 0.6, type.measure="auc", keep=TRUE, nfold = 10)
cvfit0$index
plot(cvfit0)
```
