...
* Data Wrangling
    + Missing Values 已完成，就是fill的部分看要不要优化
    + Outliers Detection 基本定义完成，detection的方法未定
    + 暂时想不到还有啥，到时候可以再加
...
```{r Check missing values by column}
    ...
    telecom <- raw_data %>%
        select(-c(numbcars, HHstatin, dwllsize, ownrent, dwlltype, lor,income, 
                  adults, infobase)) %>%
        select(-prizm_social_one) #remove umrelated col with high missing rate
    
    #Filling other calculated missing data with mean values
    #我暂时用的mean，如果后面不合适可以改成median、knn那些
    telecom$hnd_webcap = na.replace(telecom$hnd_webcap, 'UNKW') 
    na_fill_mean = function(x){
        na.fill(x,mean(x,na.rm=T))
    } 
    telecom$avg6qty = na_fill_mean(telecom$avg6qty) 
    telecom$avg6rev = na_fill_mean(telecom$avg6rev) 
    telecom$avg6mou = na_fill_mean(telecom$avg6mou)
    telecom$change_mou = na_fill_mean(telecom$change_mou)
    telecom$change_rev = na_fill_mean(telecom$change_rev)
    telecom$rev_Mean = na_fill_mean(telecom$rev_Mean)
    telecom$totmrc_Mean = na_fill_mean(telecom$totmrc_Mean)
    telecom$da_Mean = na_fill_mean(telecom$da_Mean)
    telecom$ovrmou_Mean = na_fill_mean(telecom$ovrmou_Mean)
    telecom$ovrrev_Mean = na_fill_mean(telecom$ovrrev_Mean)
    telecom$vceovr_Mean = na_fill_mean(telecom$vceovr_Mean)
    telecom$datovr_Mean = na_fill_mean(telecom$datovr_Mean)
    telecom$roam_Mean = na_fill_mean(telecom$roam_Mean)
    telecom$mou_Mean = na_fill_mean(telecom$mou_Mean)
    
    #check col missing data again
    telecom %>% 
        miss_var_summary() %>%
        filter(pct_miss > 0)
```

```{r remove missing value by mice}
telecom1 <- telecom[, -77]
...
```{r Check missing values by rows}
    row_na_idx = which(rowMeans(is.na(telecom))!= 0)
    length(row_na_idx)
    #There are 2587 rows that have some missing values. 
    #It is too small compared to the total number of rows we will remove them
    telecom = telecom[-row_na_idx,]
    
    #check row missing values again
    which(is.na(telecom))
```

* Outliers Detection
Subset different type of variables
```{r Subset different type of variables}
    #check data structure
    str(telecom)
    
    #subset variables
    var_cate = as.data.frame(sapply(telecom, typeof))
    colnames(var_cate) = "data_type"
    
    num_col <- var_cate %>%
        filter(data_type == "double") %>%
        rownames()
    ...
    tele_num = telecom %>% select(num_col)
    
    #box plots
    plot_list = list()
    for (i in 1:nrow(tele_num)){
        p = ggplot(data = telecom, 
           aes(y = unlist(tele_num[,i]), x = colnames(tele_num[,i])))+
    ...
    Rmisc::multiplot(plotlist = plot_list, layout = matrix(1:75, nrow = 15))
    
    #method1： outlier detection
    #好处是方便，直接找出所有异常值
    #坏处是一刀切，没有办法判定异常的原因，直接就remove
    detect_outliers = function(df,features){
        outlier_idx = list()
        for (c in features){
            Q1 = quantile(df[c], 0.25)
            Q3 = quantile(df[c], 0.75)
            outlier_step = IQR(df[c], na.rm = T) * 1.5
            outlier_list_col = which(df[(df[c] < Q1 - outlier_step) | 
                                            (df[c] > Q3 + outlier_step)])
        }
        outlier_idx = list(outlier_idx, outlier_list_col)
    }
    outlier_idx = table(outlier_idx)
    return (outlier_idx)
    
    #method2: 直接通过肉眼判断？？？？
    summary(tele_num) #max跟madian/mean 相差比较多的那种
    #好处是或许可以解释异常值原因再来判定要不要remove
    #坏处是比较麻烦，variables数量比较多，要一个一个来
    
    #Method3: 通过聚类进行异常值检测
    #https://blog.csdn.net/kicilove/article/details/76260350
    #也是比较麻烦得一个一个来，但是应该准确率比较高
```
