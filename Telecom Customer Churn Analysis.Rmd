...
#ROC
library(glmnet)
cvfit1 = cv.glmnet(data.matrix(X_train), y_train, family = "binomial", alpha=0.5, nlambda = 26, type.measure="auc", keep=TRUE)
cvfit1$index
...
cmat2 = confusion.glmnet(cvfit1, newx = data.matrix(X_test), newy = y_test)
```
...
```{r}
fullmod = glm(churn~., family = binomial, data = telecom1)
backward.aic = step(fullmod, direction = "backward", trace = 0)
